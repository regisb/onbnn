...
typedef mplsh_custom::FloatNnIndex Index;
...
onbnn::Object load_object( const string& img_name, const string& dir_path, const vector<string>& channels, 
                           const vector<Index*>& index_n, const vector<Index*>& index_p)
{
  // Create an object
  onbnn::Object obj;
...
int main(int argn, char* argc[])
{
  ...
  cout << "(1a/6) Loading negative images from: " << path_n << endl;
  cout << "(1b/6) Loading positive images from: " << path_p << endl;
  ...
  cout << "(2/6) Loading reference points..." << endl;
  ...
  vector<Index*> index_n(num_channels), index_p(num_channels);
  for(int c = 0; c < num_channels; c++)
  {
    // Not very clean, but this allows us to find the 
    //dimensionality of a channel
    int dim = points_n[c][0].size();
    index_n[c] = new Index(dim);
    index_p[c] = new Index(dim);

    cout << "(3a/6) Building negative index for channel " << c << "..." << endl;
    index_n[c]->build(points_n[c]);
    cout << "(3b/6) Building positive index for channel " << c << "..." << endl;
  ...
  cout << "(4/6) NN-distance for parameter learning..." << endl;
  ...
  cout << "(5/6) Training classifier..." << endl;
  classif.train();
  ...
  cout << "(6/6) Final step: Predicting test labels..." << endl;
